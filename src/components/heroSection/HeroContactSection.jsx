...
const HeroContactSection = () => {
  ...
  const handleResumeDownload = async () => {
    // Only attempt the canonical served resume URL. No fallbacks per user request.
    try {
      setResumeError(false);
      let resumeUrl = import.meta.env.RESUME_PATH || "/resume.pdf";

      const looksLikeLocalFile = typeof resumeUrl === "string" && (
        resumeUrl.startsWith("file:") || /^(?:[a-zA-Z]:\\|\\\\)/.test(resumeUrl) || resumeUrl.match(/^[a-zA-Z]:\//)
      );
      if (looksLikeLocalFile) {
        // Use the canonical public path instead of local file paths
        resumeUrl = "/resume.pdf";
      }

      // Confirm the URL exists by attempting a HEAD request. If not OK, show the specific error message
      try {
        ...
        // Prefer to verify the content-type is PDF to avoid saving an HTML page as .pdf
        const ct = resp.headers.get("content-type") || "";
        if (!/pdf/i.test(ct)) {
          // Not a PDF according to headers — treat as error to avoid corrupt download
      ...
      // Open the PDF in a new tab for in-browser viewing (don't force a download).
      // For browser PDF viewers this will keep the URL as http(s)://... instead of a local file:// path.
  ...
  {/* Keep icons row and button stacked below on all breakpoints */}
  <div className="flex flex-col items-center lg:items-start gap-6 w-full justify-center">

          <div className="flex flex-wrap justify-center lg:justify-start gap-5 items-center w-full">
          ...
          <div className="flex flex-col gap-3 w-full max-w-[220px] items-center lg:items-start lg:mx-0"> 
            <button
              onClick={handleResumeDownload}
              className="group relative w-full md:w-[200px] lg:w-[220px] py-2.5 px-5 bg-gradient-to-r from-purple-600 via-pink-600 to-orange-600 text-white font-semibold rounded-2xl hover:from-purple-700 hover:via-pink-700 hover:to-orange-700 transition-all duration-300 shadow-lg hover:shadow-xl text-center overflow-hidden mx-auto lg:mx-0"
            >
            ...
            >
              ...
              {resumeError && (
                <p className="text-sm text-red-500 mt-2">Error: Failed to load PDF document.</p>
              )}
          </div>
